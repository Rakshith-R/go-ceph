...
func TestFileBlockDiff(t *testing.T) {
	...
	randData := make([]byte, 4*1024*1024) // 4MB buffer
	...
	// Write 4MB random data to the file at 2MB offset.
	f1, err = mount.Open(f1path, os.O_RDWR, 0644)
	assert.NoError(t, err)
	assert.NotNil(t, f1)
	_, err = f1.Seek(int64(2*1024*1024), SeekSet)
	assert.NoError(t, err)
	_, err = f1.Write(randData)
	assert.NoError(t, err)
	...
	// 1MB at offset 2MB has changed between snap3 and snap4.
	changedBlocks = getChangedBlocks(t, mount, subVolRootPath, relPath, snap3, snap4)
	assert.NotNil(t, changedBlocks)
	assert.Equal(t, len(*changedBlocks), 2)
	for _, cb := range *changedBlocks {
		t.Logf("Changed block: offset=%d len=%d", cb.Offset, cb.Len)
	}
	assert.Equal(t, (*changedBlocks)[0].Offset, uint64(2*1024*1024))
	assert.Equal(t, (*changedBlocks)[0].Len, uint64(2*1024*1024))
	assert.Equal(t, (*changedBlocks)[1].Offset, uint64(4*1024*1024))
	assert.Equal(t, (*changedBlocks)[1].Len, uint64(2*1024*1024))

	// Shrink the file to 2MB.
	f1, err = mount.Open(f1path, os.O_RDWR, 0644)
	assert.NoError(t, err)
	assert.NotNil(t, f1)
	assert.NoError(t, f1.Truncate(2*1024*1024))
	// // write 32 1 byte random data at offset 0 with gap of 20 bytes
	// for i := 0; i < 32; i++ {
	// 	_, err = f1.WriteAt(randData[i:i+1], int64(i*21))
	// 	assert.NoError(t, err)
	...
	snap5 := "Snap5"
	err = fsa.CreateSubVolumeSnapshot(volume, NoGroup, subname, snap5)
	assert.NoError(t, err)
	defer func() {
		err := fsa.RemoveSubVolumeSnapshot(volume, NoGroup, subname, snap5)
		assert.NoError(t, err)
	}()

	// 1MB at offset 2MB has changed between snap3 and snap4.
	changedBlocks = getChangedBlocks(t, mount, subVolRootPath, relPath, snap4, snap5)
	assert.NotNil(t, changedBlocks)
	// assert.Equal(t, len(*changedBlocks), 2)
	for _, cb := range *changedBlocks {
		t.Logf("Changed block: offset=%d len=%d", cb.Offset, cb.Len)
...
func getChangedBlocks(t *testing.T,
	mount *MountInfo,
	rootPath, relPath, snap1, snap2 string) *[]CBlock {
	fileBlockDiffInfo, err := FileBlockDiffInit(mount, rootPath, relPath, snap1, snap2)
	assert.NoError(t, err)
	changedBlocksList := make([]CBlock, 0)
	defer func() {
		assert.NoError(t, fileBlockDiffInfo.Close())
	}()
	for {
		noMoreEntries, fileChangedBlocksInfo, err := fileBlockDiffInfo.ReadFileBlockDiff()
		assert.NoError(t, err)
		if fileChangedBlocksInfo == nil || fileChangedBlocksInfo.NumBlocks == 0 {
			break
		}
		t.Log("Got changed blocks info:", fileChangedBlocksInfo.NumBlocks)
		for _, Cblock := range fileChangedBlocksInfo.CBlocks {
			changedBlocksList = append(changedBlocksList, Cblock)
			t.Logf("Changed block: offset=%d len=%d", Cblock.Offset, Cblock.Len)
		}
		if noMoreEntries {
			break
		}
